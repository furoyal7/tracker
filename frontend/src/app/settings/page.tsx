...
export default function SettingsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (pin) {
        if (!/^\d{6}$/.test(pin)) {
           toast.error(t('settings.pinLengthError'));
           setIsSaving(false);
           return;
        }
      ...
      toast.error(error.message || t('settings.settingsError'));
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <MainLayout>
      <div className="flex flex-col space-y-8 pb-10">
        ...
        <div className="flex items-center justify-between">
           <div>
              <h2 className="text-lg font-black tracking-tight text-slate-900 uppercase">{t('settings.title')}</h2>
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">{t('settings.subtitle')}</p>
           </div>
           <Button 
             onClick={handleSave} 
             isLoading={isSaving} 
        ...
        <div className="flex flex-col space-y-6">
          ...
          <section className="space-y-4">
             <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 px-2">{t('settings.profileIdentity')}</h3>
                ...
                <div className="w-full space-y-5">
                   ...
                   <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center px-1">
                        <Info size={12} className="mr-1.5" /> {t('settings.bio')}
                      </label>
                      <textarea 
          ...
          {/* Identity & Registry */}
          <section className="space-y-4">
             <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 px-2">{t('settings.accountRegistry')}</h3>
          ...
          <section className="space-y-4">
             <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 px-2">{t('settings.businessLogic')}</h3>
             <div className="bg-white rounded-[2rem] p-6 border border-slate-50 shadow-sm space-y-2">
                <ProToggle 
                  label={t('settings.stockAuto')} 
                  sublabel={t('settings.stockAutoSub')}
                  enabled={autoStock}
                  onToggle={() => setAutoStock(!autoStock)}
                />
                <div className="h-px bg-slate-50 mx-1" />
                <ProToggle 
                  label={t('settings.realtimeProfit')} 
                  sublabel={t('settings.realtimeProfitSub')}
                  enabled={realtimeProfit}
                  onToggle={() => setRealtimeProfit(!realtimeProfit)}
                />
                <div className="h-px bg-slate-50 mx-1" />
                <ProToggle 
                  label={t('settings.globalSync')} 
                  sublabel={t('settings.globalSyncSub')}
                  enabled={autoSync}
          ...
          <section className="space-y-4">
             <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 px-2">{t('settings.systemDiagnostic')}</h3>
             <div className="bg-slate-950 rounded-[2.5rem] p-8 flex flex-col space-y-6 relative overflow-hidden shadow-2xl shadow-slate-200">
                ...
                <div className="flex items-center justify-between relative z-10">
                   <div className="flex items-center space-x-4">
                      ...
                      <div>
                         <p className="text-sm font-black text-white uppercase tracking-tight leading-none">{t('settings.systemHealth')}</p>
                         <p className="text-[9px] font-black text-blue-400/60 uppercase tracking-widest mt-2">{t('settings.latency')}: 24ms (Optimal)</p>
                      </div>
                   </div>
                ...
                <div className="grid grid-cols-2 gap-3 pt-2 relative z-10">
                   <div className="p-4 bg-white/5 rounded-2xl border border-white/5">
                      <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">{t('settings.endpoints')}</p>
                      <p className="text-sm font-bold text-white leading-none">12 Active</p>
                   </div>
                   <div className="p-4 bg-white/5 rounded-2xl border border-white/5">
                      <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">{t('settings.encLevel')}</p>
          ...
          {/* Communication Hub */}
          <section className="space-y-4">
             <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 px-2">{t('settings.notifications')}</h3>
             <div className="bg-white rounded-[2rem] border border-slate-50 shadow-sm overflow-hidden">
                ...
                <button className="w-full flex items-center justify-between p-5 active:bg-slate-50 transition-all text-left">
                   <div className="flex items-center space-x-3">
                      ...
                      <div>
                         <p className="text-[11px] font-black text-slate-900 uppercase tracking-tight leading-none">{t('settings.localeRegion')}</p>
                         <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mt-1.5">{i18n.language === 'am' ? 'ET-AM' : 'US-EN'} ($) UTC+3</p>
                      </div>
                   </div>
          ...
          <section className="space-y-4">
             <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 px-2">{t('settings.privacyAccess')}</h3>
             <div className="bg-white rounded-[2rem] border border-slate-50 shadow-sm overflow-hidden">
                <button className="w-full flex items-center justify-between p-5 active:bg-slate-50 transition-all text-left">
                   <div className="flex items-center space-x-3">
                      ...
                      <p className="text-[11px] font-black text-slate-900 uppercase tracking-tight">{t('settings.accessControl')}</p>
                   </div>
                   <ChevronRight size={14} className="text-slate-300" />
                </button>
                <div className="h-px bg-slate-50 mx-4" />
                ...
                <div className="p-5 space-y-4">
                   <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                         ...
                         <div>
                            <p className="text-[11px] font-black text-slate-900 uppercase tracking-tight leading-none">{t('settings.securityKey')}</p>
                            <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mt-1.5 leading-none">{t('settings.securityKeySub')}</p>
                         </div>
                      </div>
                   </div>
                   <div className="relative">
                ...
                <button className="w-full flex items-center justify-between p-5 active:bg-slate-50 transition-all text-left">
                   <div className="flex items-center space-x-3">
                      ...
                      <p className="text-[11px] font-black text-slate-900 uppercase tracking-tight">{t('settings.hardwareReset')}</p>
                   </div>
                   <ChevronRight size={14} className="text-slate-300" />
                </button>
             </div>
          ...
          <div className="pt-6">
             ...
             >
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.2em]">{t('settings.terminateSession')}</span>
             </button>
          </div>
       </div>
